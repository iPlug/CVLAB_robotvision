...
from utils.grab_proposal import EdgeAwareGrabPointProposer
...
class FrameSequenceProcessor:
    def __init__(
        ...
        self.grab_proposer = EdgeAwareGrabPointProposer(
            edge_detection_method='normal_variation',  # Most robust
            min_distance_from_edge=0.015,     # 1.5cm from edges
            normal_variation_threshold=0.3     # Sensitivity to normal changes
        )
    ...
    def process_single_frame_optimized(self, file_path):
        ...
        for i, cluster_id in enumerate(range(max_label + 1)):
            ...
            if (
                ...
                if self.grab_proposer:
                    for i, cluster_pcd in enumerate(valid_clusters):
                        grab_points = self.grab_proposer.propose_grab_points_edge_aware(
                            cluster_pcd, filter_orientation=True
                        )
                        ...
                        if grab_points:
                            ...
                            # Create spheres (circles in 3D) to represent grab points
                            markers = []
                            for grab_point in grab_points:
                                sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.01)  # 1cm sphere radius
                                sphere.translate(grab_point['position'])
                                sphere.paint_uniform_color([1, 0, 0])  # Red color for grab points
                                markers.append(sphere)
                            grab_markers.extend(markers)

                # CHANGE your return statement to:
        return valid_clusters, valid_bboxes, text_labels, pcd, all_grab_points, grab_markers
    ...
    def process_all_frames(self, start_frame=0, end_frame=None):
        ...
        for i, file_path in enumerate(ply_files):
            ...
            clusters, bboxes, text_labels, original_pcd, grab_points, grab_markers  = self.process_single_frame_optimized(file_path)
            ...
            if self.visualize and vis is not None:
                ...
                if len(filtered_points) > 0:
                    ...
                    for marker in grab_markers:
                        vis.add_geometry(marker, reset_bounding_box=False)
                        markers_geom.append(marker)
