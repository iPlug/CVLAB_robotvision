...
class VirtualFixedCameraTracker:
    """
    Virtual fixed camera tracking application with on-demand pointing.
    ...
    def _grasp_and_return(self):
        ...
        print(f"Target: ({camera_position[0]:.1f}, {camera_position[1]:.1f}, {camera_position[2]:.1f}) mm")
        print(f"{'='*60}")
        ...
        try:
            if self.robot_controller.async_mode_enabled:
                ...
                robot_position = self.robot_controller.transform_camera_to_robot(camera_position)
    ...
    def run(self):
        ...
        print("\nVirtual Fixed Camera Tracking Started!")
        print("=" * 50)
        print("Controls:")
        print("  'q' - Quit")
        print("  'h' - Toggle help")
    ...
    def _extract_object_position(self, obj: Dict[str, Any]) -> Optional[List[float]]:
        """Extract 3D position from object in camera coordinates (returns in mm)."""
        try:
            if 'center_mm' in obj:
                # center_mm format: [x, y, z] = [side, height, forward] in mm
                center_mm = obj['center_mm']
                # Reorder to [forward, side, height] and keep in mm
                return [center_mm[2], center_mm[0], center_mm[1]]
            elif 'center' in obj:
                # center format: [x, y, z] = [side, height, forward] in meters
                center = obj['center']
                # Convert to mm and reorder to [forward, side, height]
                return [center[2]*1000, center[0]*1000, center[1]*1000]
            else:
                return None
        except Exception as e:
    ...
    def _add_object_detection_overlay(self, image: np.ndarray, detected_objects: List[Dict[str, Any]]):
        ...
        for i, obj in enumerate(detected_objects[:3]):  # Show first 3 objects
            position = self._extract_object_position(obj)
            if position:
                # Format: (forward, side, height) to match original system, in mm
                info_text = f"Obj{i+1}: ({position[0]:.0f}, {position[1]:.0f}, {position[2]:.0f})mm"
...
def main():
    ...
    parser.add_argument('--transform', default='camera_robot_transform.npy',
