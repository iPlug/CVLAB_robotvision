...
import sys, os, cv2, numpy as np, pyrealsense2 as rs, time
from pathlib import Path
from sklearn.linear_model import RANSACRegressor
from sklearn.cluster import DBSCAN
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from utils_modules import RobotUtils, CameraUtils, CoordinateUtils, DetectionUtils, PointCloudUtils, VisualizationUtils
...
class EnhancedLidarPredictor:
    ...
    def __init__(self, config_path=None, prediction_offset=None):
        """Initialize the enhanced real-time LiDAR object detection system.
        
        Args:
            config_path: Path to configuration directory (default: same directory as main.py)
        ...
        """
        # Set config_path to the directory where main.py is located if not specified
        ...
        self.clustering_min_samples = 100         # Minimum points for object cluster
        self.clustering_max_samples = 5000        # Maximum points for object cluster (filter hands)
        ...
        self.point_cloud_utils = PointCloudUtils()
        ...
        self.last_detection_objects = []  # Store for grasping
        self.last_detection_results = {}  # Store complete detection results for UI during robot actions
        
        # Initialize robot motion controller for LLM integration (after movement_speed is defined)
        self.robot_motion_controller = RobotMotionController(
            enhanced_robot_controller=self.enhanced_robot_controller,
    ...
    # LLM Integration - Robot Motion Controller Wrapper Methods
    
    def _grasp_and_return_via_controller(self):
        """Wrapper for grasping closest object via RobotMotionController."""
        ...
        # Extract position and use robot motion controller
        camera_position_m = self._extract_object_position(closest_object)
        if camera_position_m is not None:
            self.robot_motion_controller.grasp_object(camera_position_m=camera_position_m)
    
    def _grasp_specific_object_via_controller(self, object_id: int):
        """Wrapper for grasping specific object via RobotMotionController."""
        if not self.last_detection_objects:
            print(f"No objects detected for grasping object #{object_id}")
            return
        
        if object_id < 1 or object_id > len(self.last_detection_objects):
            print(f"Object #{object_id} not found (available: 1-{len(self.last_detection_objects)})")
            return
        
        # Get target object (1-based indexing)
        target_object = self.last_detection_objects[object_id - 1]
        camera_position_m = self._extract_object_position(target_object)
        
        if camera_position_m is not None:
            print(f"Targeting object #{object_id}")
            self.robot_motion_controller.grasp_object(object_id=object_id, camera_position_m=camera_position_m)
    ...
    def _add_tracking_info_overlay(self, image, results, robot_pose, position_in_base):
        ...
        cv2.putText(image, "Controls: 'g'=Grasp, 'w'/'s'=Tilt, 'f'=Forward, 'r'=Return, 'q'=Quit", 
    ...
    def detect_tabletop_object(self, color_frame, depth_frame):
        ...
        try:
            ...
            if self.grasping_in_progress:
                return self.create_empty_detection_result()
            # Use surface-based detection only (more robust and consistent)
    ...
    def find_all_significant_clusters(self, object_points, min_cluster_size=15, max_cluster_size=None):
        ...
        # Use instance parameter if not provided
        if max_cluster_size is None:
            max_cluster_size = self.clustering_max_samples
        ...
        for label in unique_labels:
            ...
            # Only include clusters within size range (exclude hands and tiny noise)
            if min_cluster_size <= cluster_size <= max_cluster_size:
                ...
                print(f"Filtered out large cluster: {cluster_size} points (likely hand or background)")
    ...
    def handle_enhanced_user_input(self):
        ...
        # Enhanced robot controls using RobotMotionController
        elif key == ord('g'):  # Grasp closest object
            if not self.robot_motion_controller.grasping_in_progress:
                self._grasp_and_return_via_controller()
            else:
                print("Robot action already in progress")
        ...
        # Number keys for specific object grasping
            ...
            if not self.robot_motion_controller.grasping_in_progress:
                self._grasp_specific_object_via_controller(object_id)
            else:
                print("Robot action already in progress")
        ...
        # Standard robot position controls
            ...
            self.robot_motion_controller.move_to_home()
            
        elif key == ord('h'):  # Home position (same as 'r' for compatibility)
            self.robot_motion_controller.move_to_home()
            
        elif key == ord('s'):  # Stop current robot action
            self.robot_motion_controller.stop_current_action()
            
        elif key == ord('o'):  # Open gripper manually
            self.robot_motion_controller.open_gripper()
            
        elif key == ord('c'):  # Close gripper manually 
            self.robot_motion_controller.close_gripper()
        ...
        elif key == ord('?') or key == ord('/'):  # Help for LLM integration
            self._print_llm_help()
        
        return True  # Continue main loop
    
    def _print_llm_help(self):
        """Print help information for LLM integration."""
        print("\n" + "="*60)
        print("LLM GRABBER - ROBOT CONTROL INTERFACE")
        print("="*60)
        print("\nKEYBOARD CONTROLS:")
        print("  GRASPING:")
        print("    'g'       - Grasp closest detected object")
        print("    '1'-'9'   - Grasp specific object by ID")
        print("  ROBOT CONTROL:")
        print("    'h' or 'r'- Move to home/observation position")
        print("    's'       - STOP all robot movements (emergency)")
        print("    'o'       - Open gripper manually")
        print("    'c'       - Close gripper manually")
        print("  CAMERA:")
        print("    'w'/'a'   - Tilt camera up/down")
        print("    'f'       - Look forward")
        print("  SYSTEM:")
        print("    'q'/ESC   - Quit application")
        print("    '?'/'/'   - Show this help")
        
        print("\nROBOT MOTION FUNCTIONS (for LLM integration):")
        try:
            from robot_motions import get_available_robot_functions
            functions = get_available_robot_functions()
            for func_name, func_info in functions.items():
                print(f"  {func_name}() - {func_info['description']}")
        except Exception:
            print("  [Robot motion functions not available]")
        
        print(f"\nROBOT STATUS:")
        if hasattr(self, 'robot_motion_controller'):
            status = self.robot_motion_controller.get_robot_status()
            for key, value in status.items():
                if key != 'timestamp':
                    print(f"  {key}: {value}")
        
        print("\nLLM INTEGRATION READY:")
        print("  - Robot motion functions extracted and callable")
        print("  - State machine handles non-blocking operations") 
        print("  - Safe error handling and status reporting")
        print("  - Ready for natural language command processing")
        print("="*60 + "\n")
    ...
    def run(self):
        print("Starting LiDAR Real-time Prediction System...")
        ...
        try:
            while True:
                ...
                # Process robot motion controller state machine (for LLM integration)
                self.robot_motion_controller.process_grasping_state_machine()
                
                # Process non-blocking grasping state machine (legacy system)
                ...
                if self.grasping_in_progress or self.returning_to_observer:
                ...
                try:
                    ...
                    if robot_pose:
                        ...
                        if detection_results['valid_detection'] and detection_results['objects']:
                            ...
                            if not (self.grasping_in_progress or self.returning_to_observer):
...
def main():
    ...
    parser.add_argument('--config-path', default=None,
                       help='Path to configuration directory (default: same directory as main.py)')
    ...
    safety_status = "ENABLED" if args.enable_safety else "DISABLED"
