#!/usr/bin/env python3
"""
Checkerboard-Based Eye-in-Hand Calibration

This script performs high-precision camera-robot calibration using checkerboard patterns
for eye-in-hand setups. The robot-mounted camera observes a fixed checkerboard pattern
...
Usage:
    python checkerboard_eye_in_hand.py [options]
...
Examples:
    ...
    python checkerboard_eye_in_hand.py --config checkerboard_quick --pattern 8 6
    
    # Precise calibration with custom square size
    python checkerboard_eye_in_hand.py --config checkerboard_precise --square-size 20.0
    
    # Custom pattern position on table
    python checkerboard_eye_in_hand.py --pattern-position 200 0 0
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Checkerboard-based eye-in-hand calibration",
...
Examples:
    ...
    # Quick calibration
    python checkerboard_eye_in_hand.py --config checkerboard_quick
    
    # Custom pattern and position
    python checkerboard_eye_in_hand.py --pattern 8 6 --pattern-position 250 50 0
    
    # Verbose output with custom square size
    python checkerboard_eye_in_hand.py --square-size 30.0 --verbose
        """
    )
    ...
    parser.add_argument(
        ...
        metavar=('X', 'Y', 'Z'),
        default=[400, 0, 0],
        help='Fixed pattern position on table [x, y, z] in mm (default: 200 0 0)'
    )
    
    parser.add_argument(
        '--output-matrix',
        type=str,
        default='eye_in_hand_transform.npy',
        help='Output file for transformation matrix (default: eye_in_hand_transform.npy)'
    )
    ...
    parser.add_argument(
        '--log-file',
        type=str,
        default='eye_in_hand_calibration.log',
        help='Log file for calibration process (default: eye_in_hand_calibration.log)'
    )
...
def list_available_configs():
    ...
    print("  checkerboard_standard  - Balanced speed and accuracy (12 positions)")
    print("  checkerboard_precise   - High precision, more positions (15+ positions)")
    print("  checkerboard_quick     - Fast calibration (8 positions)")
...
def print_calibration_info(args):
    ...
    print("Checkerboard-Based Eye-in-Hand Calibration")
    print("=" * 50)
    ...
    print("2. Place checkerboard pattern at fixed position")
...
def create_custom_config(args):
    ...
    if hasattr(base_config, 'config'):
        ...
        if 'pattern_params' in base_config.config:
            base_config.config['pattern_params']['pattern_sizes'] = [tuple(args.pattern)]
            base_config.config['pattern_params']['square_size_mm'] = args.square_size
            base_config.config['pattern_params']['default_pattern'] = tuple(args.pattern)
...
def main():
    ...
    try:
        ...
        print("\\nInitializing eye-in-hand checkerboard calibration system...")
        ...
        print("2. Place the checkerboard pattern on the table at the specified position")
        print(f"   Pattern position: [{args.pattern_position[0]}, {args.pattern_position[1]}, {args.pattern_position[2]}] mm")
        ...
        if success:
            ...
            print("- Robot-guided object inspection")
            ...
            print("- Ensure pattern is large enough to be seen from all positions")
